...
#Machine learning models will discriminated between taxa
#Remove Megamonas, Cellvibrio and Megasphaera from this analysis (a small number of genomes was found for these genera)
glycosidase_data_numeric <- glycosidase_data_numeric[-which(glycosidase_data_numeric$Taxa == "Megamonas"),]
glycosidase_data_numeric <- glycosidase_data_numeric[-which(glycosidase_data_numeric$Taxa == "Cellvibrio"),]
glycosidase_data_numeric <- glycosidase_data_numeric[-which(glycosidase_data_numeric$Taxa == "Megasphaera"),]
...
#Export logitboost model as an R object
saveRDS(logitboost, "logitboost.rds")
#Read logitboost model stored in an R object
...
#Export glmnet model as an R object
saveRDS(glmnet, "glmnet.rds")
#Read glmnet model stored in an R object
...
#Export rf model as an R object
saveRDS(rf, "rf.rds")
#Read rf model stored in an R object
rf <- readRDS("rf.rds")
...
#Export ann model as an R object
saveRDS(ann, "ann.rds")
#Read ann model stored in an R object
ann <- readRDS("ann.rds")
...
#Set row names
rownames(model_metrics) <- paste(c(
                              rep("PLS - Train phase",nrow(pls_confusion_matrix_train$byClass)),
                              rep("LOGITBOOST - Train phase", nrow(pls_confusion_matrix_train$byClass)),
                              rep("GLMNET - Train phase", nrow(pls_confusion_matrix_train$byClass)),
                              rep("RF - Train phase", nrow(pls_confusion_matrix_train$byClass)),
                              rep("ANN - Train phase", nrow(pls_confusion_matrix_train$byClass)),
                              rep("PLS - Test phase", nrow(pls_confusion_matrix_train$byClass)),
                              rep("LOGITBOOST - Test phase", nrow(pls_confusion_matrix_train$byClass)),
                              rep("GLMNET - Test phase", nrow(pls_confusion_matrix_train$byClass)),
                              rep("RF - Test phase", nrow(pls_confusion_matrix_train$byClass)),
                              rep("ANN - Test phase", nrow(pls_confusion_matrix_train$byClass))), 
                              rownames(model_metrics))
#Format row names
rownames(model_metrics) <- gsub("Class..", "", rownames(model_metrics))
rownames(model_metrics) <- gsub("[.].*", "", rownames(model_metrics))
#set column names
...
rf_importance <- abs(rf_importance[,1:(ncol(rf_importance)-2), drop=F])
#Transform variable importance coefficients to relative percentages (%)
...
ann_importance_label_3 <- olden(ann$finalModel, bar_plot=F, out_var=paste0("Output_", ann$levels[[3]]))
ann_importance_label_4 <- olden(ann$finalModel, bar_plot=F, out_var=paste0("Output_", ann$levels[[4]]))
ann_importance_label_5 <- olden(ann$finalModel, bar_plot=F, out_var=paste0("Output_", ann$levels[[5]]))
ann_importance_label_6 <- olden(ann$finalModel, bar_plot=F, out_var=paste0("Output_", ann$levels[[6]]))
ann_importance_label_7 <- olden(ann$finalModel, bar_plot=F, out_var=paste0("Output_", ann$levels[[7]]))
ann_importance <- cbind.data.frame(ann_importance_label_1, ann_importance_label_2,
                                          ann_importance_label_3, ann_importance_label_4,
                                          ann_importance_label_5, ann_importance_label_6,
                                          ann_importance_label_7)
#Format row names
rownames(ann_importance) <- gsub("Input_", "", rownames(ann_importance))
rownames(ann_importance) <- gsub("_", " ", rownames(ann_importance))
#Format col names
colnames(ann_importance) <- c(ann$levels[[1]], ann$levels[[2]], ann$levels[[3]],
                              ann$levels[[4]], ann$levels[[5]], ann$levels[[6]],
                              ann$levels[[7]])
#Transform variable importance coefficients to relative percentages (%)
ann_importance <- round(ann_importance*100/max(ann_importance), digits=2)


#Row names from these variable importance datasets are identical
rownames(pls_importance) == rownames(logitboost_importance)
rownames(logitboost_importance) == rownames(glmnet_importance)
rownames(glmnet_importance) == rownames(rf_importance)
rownames(rf_importance) == rownames(ann_importance)
#Create a new table containing variable importance coefficients for all models
model_variable_importance <- cbind.data.frame(pls_importance,
                                              logitboost_importance,
                                              glmnet_importance,
                                              rf_importance,
                                              ann_importance)
#Set column names
names(model_variable_importance) <- paste(c(rep("PLS importance (%):", nrow(pls_confusion_matrix_train$byClass)),
                                      rep("LOGITBOOST importance (%):", nrow(pls_confusion_matrix_train$byClass)),
                                      rep("GLMNET importance (%):", nrow(pls_confusion_matrix_train$byClass)),
                                      rep("RF importance (%):", nrow(pls_confusion_matrix_train$byClass)),
                                      rep("ANN mportance (%):", nrow(pls_confusion_matrix_train$byClass))),
                                      names(model_variable_importance))
#Order table
model_variable_importance <- model_variable_importance[order(factor(rownames(model_variable_importance), 
                                                                  levels=unique(mixedsort(rownames(model_variable_importance))))),]
#Write xlsx file
...
differences_Bacteroides <- differences_glycosidases$Bacteroides
differences_Bifidobacterium <- differences_glycosidases$Bifidobacterium
differences_Clostridium <- differences_glycosidases$Clostridium
differences_Eubacterium <- differences_glycosidases$Eubacterium
differences_Lactobacillus <- differences_glycosidases$Lactobacillus
differences_Prevotella <- differences_glycosidases$Prevotella
differences_Roseburia <- differences_glycosidases$Roseburia
...
#Glycosidases more abundant in different taxa
differences_Bacteroides <- differences_Bacteroides[order(factor(differences_Bacteroides$Domain, 
                                                          levels=unique(mixedsort(differences_Bacteroides$Domain)))),]
differences_Bifidobacterium <- differences_Bifidobacterium[order(factor(differences_Bifidobacterium$Domain, 
                                                                levels=unique(mixedsort(differences_Bifidobacterium$Domain)))),]
differences_Clostridium <- differences_Clostridium[order(factor(differences_Clostridium$Domain, 
                                                                levels=unique(mixedsort(differences_Clostridium$Domain)))),]
differences_Eubacterium <- differences_Eubacterium[order(factor(differences_Eubacterium$Domain, 
                                                                levels=unique(mixedsort(differences_Eubacterium$Domain)))),]
differences_Lactobacillus <- differences_Lactobacillus[order(factor(differences_Lactobacillus$Domain, 
                                                                levels=unique(mixedsort(differences_Lactobacillus$Domain)))),]
differences_Prevotella <- differences_Prevotella[order(factor(differences_Prevotella$Domain, 
                                                                levels=unique(mixedsort(differences_Prevotella$Domain)))),]
differences_Roseburia <- differences_Roseburia[order(factor(differences_Roseburia$Domain, 
                                                                levels=unique(mixedsort(differences_Roseburia$Domain)))),]
#Write xlsx files
xlsx::write.xlsx(summary_glycosidases, "Summary_glycosidases.xlsx")
xlsx::write.xlsx(differences_Bacteroides, "Differences_Bacteroides.xlsx")
xlsx::write.xlsx(differences_Bifidobacterium, "Differences_Bifidobacterium.xlsx")
xlsx::write.xlsx(differences_Clostridium, "Differences_Clostridium.xlsx")
xlsx::write.xlsx(differences_Eubacterium, "Differences_Eubacterium.xlsx")
xlsx::write.xlsx(differences_Lactobacillus, "Differences_Lactobacillus.xlsx")
xlsx::write.xlsx(differences_Prevotella, "Differences_Prevotella.xlsx")
xlsx::write.xlsx(differences_Roseburia, "Differences_Roseburia.xlsx")
